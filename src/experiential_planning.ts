...
import { GoogleGenAI, Content, Type, FunctionCallingConfigMode } from '@google/genai';
...
export interface PlanStep {
  ...
  completed: boolean;
  actualObs?: any;
  actualReward?: number | { total: number; [key: string]: any };
  explanations?: string;
}
 ...
 */
export class ExperientialPlanner {
  ...
  async createPlan(
    ...
    for (let i = 0; i < maxSteps; i++) {
      ...
      const bestAction = await this.findBestNextAction(
        currentState,
        goal,
        possibleActions,
        searchDepth
      ...
      const prediction = await this.worldModel.predictNextState(
        currentState,
        bestAction.action
      );
    ...
    plan.timeframe.estimatedCompletion = new Date(
      plan.timeframe.start.getTime() + (plan.steps.length * 600000) // 10 minutes per step
    );
   ...
   */
  private async findBestNextAction(
    currentState: any,
    goal: Goal,
    possibleActions: any[],
    depth: number = 3
  ): Promise<{action: any, value: number, reasoning?: string} | null> {
      ...
      possibleActions.map(async (action) => {
        try {
          ...
          if (depth > 1) {
            ...
            const nextBestAction = await this.findBestNextAction(
          ...
          const goalProgress = this.evaluateGoalProgress(
            prediction.predictedObs,
            goal
          );
          ...
          const totalValue = (
            prediction.predictedReward + // Immediate reward
   ...
   */
  private evaluateGoalProgress(state: any, goal: Goal): number {
   ...
   */
  async updatePlan(plan: Plan, stepIndex: number, actualExperience: Experience): Promise<Plan> {
    ...
    if (needsReplanning && stepIndex < plan.steps.length - 1) {
      ...
      const newRemainingSteps = await this.createPlan(
        actualExperience.next_obs,
        plan.goal,
        plan.steps.map(s => s.action), // Use same action space
      ...
      // Update plan metrics
      plan.estimatedReward = plan.steps.reduce(
        (sum, s) => {
          if (s.completed) {
  ...
  private shouldReplan(step: PlanStep): boolean {
    ...
    const rewardDeviation = Math.abs(
      (actualRewardValue - step.predictedReward) / Math.max(0.1, Math.abs(step.predictedReward))
    );
   ...
   */
  async generatePotentialActions(
    observation: any,
    goal: Goal,
    actionTypes: string[] = ['message', 'tool']
  ): Promise<any[]> {
    try {
   ...
   */
  async explainPlan(plan: Plan): Promise<string> {
    try {
      const contents: Content[] = [{
        role: 'user',
        parts: [{ text: `
          ...
          ${plan.steps.map((step, i) => 
            `Step ${i+1}: ${JSON.stringify(step.action)}\n`
          ).join('')}
        `}]
      }];
