...
// Define result interfaces for type safety
...
async function getWeather(location: string, unit: string = "celsius"): Promise<WeatherResult> {
...
async function searchNews(topic: string, limit: number = 3): Promise<NewsArticle[]> {
...
interface ToolResult {
  name: string;
  result: WeatherResult | NewsArticle[];
}
...
async function runDemo() {
  try {
    ...
    if (functionCalls.length > 0) {
      ...
      const toolResults = await Promise.all(
        functionCalls.map(async (functionCall: any) => {
          ...
          if (functionCall.name === 'get_weather') {
            const { location, unit = 'celsius' } = functionCall.args as { location: string, unit?: string };
            result = await getWeather(location, unit);
          } else if (functionCall.name === 'search_news') {
            const { topic, limit = 3 } = functionCall.args as { topic: string, limit?: number };
      ...
      const validResults = toolResults.filter((result): result is ToolResult => 
      ...
      if (validResults.length > 0) {
        ...
        let newsResults: NewsArticle[] = [];
        
        // Extract typed results
        for (const result of validResults) {
          if (result.name === 'get_weather') {
            weatherResult = result.result as WeatherResult;
          } else if (result.name === 'search_news') {
            newsResults = result.result as NewsArticle[];
          }
        }
