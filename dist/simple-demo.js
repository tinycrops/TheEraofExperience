...
async function runSimpleDemo() {
    ...
    const memory = new memory_1.VectorMemory(apiKey, 'data/simple_experience.db');
    ...
    const initialResponse = await generateResponse(model, initialObservation, []);
    ...
    const secondResponse = await generateResponseWithActions(model, secondObservation, experiences);
    ...
    const rewardValue = calculateGroundedReward(environment.user.healthMetrics, updatedUser.healthMetrics, environment.user.goal);
    ...
    const predictions = await Promise.all(potentialActions.map(async (action) => {
    ...
    predictions.sort((a, b) => (b.predictedReward * b.confidence) - (a.predictedReward * a.confidence));
    ...
    const finalResponse = await generatePlannedResponse(model, thirdObservation, experiences, bestAction);
 ...
 */
async function generateResponse(model, observation, experiences) {
 ...
 */
async function generateResponseWithActions(model, observation, experiences) {
    const context = experiences.length > 0
        ? `\n\nPast experiences: ${JSON.stringify(experiences)}`
        : '';
    const result = await model.generateContent({
 ...
 */
async function generatePlannedResponse(model, observation, experiences, bestAction) {
    const context = experiences.length > 0
        ? `\n\nPast experiences: ${JSON.stringify(experiences)}`
        : '';
    const result = await model.generateContent({
 ...
 */
function calculateGroundedReward(previous, current, goals) {
    let totalReward = 0;
    let metricCount = 0;
