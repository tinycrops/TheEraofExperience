...
class PPO {
    ...
    async calculateAdvantages(experiences) {
        ...
        const stdDev = Math.sqrt(advantages.reduce((sum, adv) => sum + Math.pow(adv - mean, 2), 0) / advantages.length);
        return advantages.map(adv => (stdDev > 1e-8) ? (adv - mean) / stdDev : adv);
    }
    // Estimate the value of a state
    async estimateValue(observation, valueModelInstance) {
        try {
            if (!observation) {
                return 0;
            }
    ...
    async act(observation) {
        ...
        const tools = [{ functionDeclarations: [experiential_agent_1.WebSearchToolSchema] }];
        let contents;
            ...
            const result = await this.model.generateContent({
                ...
                toolConfig: {
                    functionCallingConfig: {
                        ...
                        allowedFunctionNames: [experiential_agent_1.WebSearchToolSchema.name],
                    },
                },
                generationConfig: {
                    temperature: 0.7,
        ...
        try {
            ...
            for (let epoch = 0; epoch < this.options.epochs; epoch++) {
                ...
                for (const index of shuffledIndices) {
                    ...
                    const surrogate2 = Math.max(1 - this.options.clipEpsilon, Math.min(1 + this.options.clipEpsilon, ratio)) * advantage;
...
exports.PPO = PPO;
//# sourceMappingURL=rl_core.js.map