...
function createExperience(observation, action, nextObs, reward) {
    return {
        obs: observation,
        action: action,
        next_obs: nextObs,
...
async function runPlanningDemo() {
    ...
    const vectorMemory = new memory_1.VectorMemory(apiKey, 'data/planning_demo.db');
    // Initialize world model
    const worldModel = new world_model_1.WorldModel(genAI, vectorMemory, 'gemini-2.0-flash');
    ...
    const exp1 = createExperience({ message: { text: "What is quantum computing?" } }, { functionCall: { name: 'webSearch', args: { query: 'quantum computing basics' } } }, { message: { text: "Found information about quantum computing from reputable sources." } }, 0.8);
    await worldModel.update(exp1);
    // Scenario 2: Direct explanation of a topic
    const exp2 = createExperience({ message: { text: "Explain how quantum computing differs from classical computing." } }, { parts: [{ text: "Quantum computing differs from classical computing in several key ways. In classical computing, information is processed in bits (0s and 1s). In quantum computing, information is processed in quantum bits or 'qubits' that can exist in multiple states simultaneously thanks to superposition..." }] }, { message: { text: "That was helpful, thank you!" } }, 0.9);
    await worldModel.update(exp2);
    // Scenario 3: Unsuccessful web search
    const exp3 = createExperience({ message: { text: "What's the weather like today?" } }, { functionCall: { name: 'webSearch', args: { query: 'current weather' } } }, { message: { text: "I'm sorry, I couldn't retrieve the current weather information." } }, 0.2);
    await worldModel.update(exp3);
    // Scenario 4: Step-by-step tutorial
    const exp4 = createExperience({ message: { text: "How does quantum entanglement work?" } }, { parts: [{ text: "Let me explain quantum entanglement step by step:\n\n1. Entanglement is a quantum phenomenon where two or more particles become correlated in such a way that the quantum state of each particle cannot be described independently.\n\n2. When particles are entangled, whatever happens to one particle can affect the other, no matter the distance between them.\n\n3. This happens because their quantum states are fundamentally linked during the entanglement process." }] }, { message: { text: "That makes it much clearer, thanks!" } }, 0.95);
...
//# sourceMappingURL=planning-demo.js.map