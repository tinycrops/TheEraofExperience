 ...
 */
class WorldModel {
    constructor(genAI, memory, modelName = 'gemini-2.0-flash') {
        this.genAI = genAI;
        this.memory = memory;
     ...
     */
    async predictNextState(observation, proposedAction) {
        try {
...
` }]
     ...
     */
    async planActions(observation, possibleActions, depth = 2) {
        ...
        for (const action of possibleActions) {
            try {
                ...
                if (depth === 1) {
                    ...
                    const futurePlan = await this.planActions(prediction.predictedObs, possibleActions, depth - 1);
        ...
        plans.sort((a, b) => (b.totalReward * b.confidence) - (a.totalReward * a.confidence));
