...
const experience_stream_1 = require("./experience_stream");
const experiential_planning_1 = require("./experiential_planning");
...
async function runExperienceDemo() {
    ...
    const memory = new memory_1.VectorMemory(apiKey);
    // Initialize world model
    console.log("🌍 Initializing world model...");
    const worldModel = new world_model_1.WorldModel(genAI, memory);
    // Create or load an experience stream
    console.log("🌊 Creating experience stream...");
    const streamId = `demo-stream-${Date.now()}`;
    const experienceStream = await (0, experience_stream_1.getExperienceStream)(streamId, worldModel, memory);
    // Initialize the planner
    console.log("📝 Initializing experiential planner...");
    const planner = new experiential_planning_1.ExperientialPlanner(genAI, worldModel, memory);
    // Define a goal
    const healthGoal = {
 ...
 */
async function runSimulation(genAI, worldModel, experienceStream, planner, goal, streamId) {
    ...
    const potentialActions = await planner.generatePotentialActions(initialObservation, goal, possibleActionTypes.map(a => a.type));
    console.log(`Generated ${potentialActions.length} potential actions`);
    // Create a plan
    const plan = await planner.createPlan(initialObservation, goal, potentialActions, 5, // Maximum steps
    3 // Look-ahead depth
    );
    ...
    for (let i = 0; i < plan.steps.length; i++) {
        ...
        const simulatedResponse = await simulateEnvironmentResponse(environment, step.action, i);
        // Calculate grounded reward based on environment response
        const rewardResult = await (0, reward_functions_1.calcReward)(initialObservation, step.action, {
            healthMetrics: {
 ...
 */
async function simulateEnvironmentResponse(environment, action, stepIndex) {
 ...
 */
function updateEnvironment(environment, simulationResults) {
    ...
    environment.user.healthMetrics.heartRate = Math.max(65, environment.user.healthMetrics.heartRate - 0.5);
}
// Run the demo
...
exports.default = runExperienceDemo;
//# sourceMappingURL=era-of-experience-demo.js.map